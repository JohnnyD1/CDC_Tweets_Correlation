...
#df <- df[!(WEEK == 53),]
# need to concatenate WEEK and SEASON
# maybe not
#sub_cols <- c("SEASON", "WEEK")
#sub_df <- df[sub_cols]
#sub_df <- transform(sub_df,SEASONWEEK=paste0(SEASON,WEEK))
#attach(sub_df)

#df$SEASONWEEK <- SEASONWEEK
#detach(sub_df)
#attach(df)
#df$SEASONWEEK <- as.numeric(as.character(SEASONWEEK))
...
sam_point1 <- sample(1:length(df$order),1)
sam_pointx1 <- df$order[sam_point]
sam_pointy1 <- df$PERCENT.P.I[sam_point]

ggplot() + 
geom_line(data=df, aes(x=order, y=PERCENT.P.I), color="red")+
stat_smooth(df,mapping=aes(x =order , y = BASELINE), method = "lm",
  formula = y ~ poly(x, 25), color="black")+
stat_smooth(df,mapping=aes(x =order , y = THRESHOLD), method = "lm",
  formula = y ~ poly(x, 25), color="black")+
xlab("MMWR Week") +
...
geom_segment(aes(x=86-15, xend=86, y= 6.6-1, yend= 6.6),
  arrow = arrow(length = unit(0.5, "cm")))+
annotate(geom="text", x=86 - 15, 6.6-1.3, label="Seasonal Baseline", color="black",size=3)+
geom_segment(aes(x=53-15, xend=53, y=6.5+2.5, yend= 6.5), arrow = arrow(length = unit(0.5, "cm")))+
annotate(geom="text", x=53-15, 6.5+2.6, label="Epidemic Threshold", color="black", size=3)+
